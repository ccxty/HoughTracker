...
std::vector<std::vector<HoughGridArea *> *> *GridInit1() {
  ...
  for (int i = 0; i < NAlpha; i++) {
    ...
    for (int j = 0; j < ND; j++) {
      auto ptr3 = new HoughGridArea(
          AlphaMin + i * AlphaBinWidth, AlphaMin + (i + 1) * AlphaBinWidth,
...
void FillGrid1(std::vector<std::vector<HoughGridArea *> *> *gridMatrix,
  ...
  for (int ip = 0; ip < pointsList.size(); ip++) {
    ...
    for (int ia = 0; ia < gridMatrix->size(); ia++) {
      ...
      for (int id = 0; id < row->size(); id++) {
        ...
        double d1 = point->xConformal() * cos(alpha1) +
                    point->yConformal() * sin(alpha1);
        double d2 = point->xConformal() * cos(alpha2) +
                    point->yConformal() * sin(alpha2);
        if (((d1 >= grid->yMin()) && (d1 < grid->yMax())) ||
            (d2 >= grid->yMin()) && (d2 < grid->yMax())) {
          grid->CountsAddOne();
    ...
    for (int id = 0; id < row->size(); id++) {
      ...
      if (grid->counts() >= 3) {
        ...
        for (int ip = 0; ip < points->size(); ip++) {
          ...
          std::cout << flag << " " << grid->counts() << " " << point->eventID()
                    << " " << point->layerID() << " " << ia << " " << id
                    << endl;
        }
        flag++;
...
std::vector<HoughTrack *> *find_track(std::
                                          vector<std::vector<
                                              HoughGridArea *> *> *gridMatrix /*, std::vector<int> &PeakGridX, std::vector<int> &PeakGridY*/) {
    ...
    for (int id = 0; id < row->size(); id++) {
      ...
      if (grid->counts() >= 3) {
        ...
        if ((ia > 0) && (id > 0) && (ia + 1 < gridMatrix->size()) &&
          ...
          if ((grid->counts() >= counts1) && (grid->counts() >= counts2) &&
              (grid->counts() >= counts3) && (grid->counts() >= counts4) &&
              (grid->counts() >= counts5) && (grid->counts() >= counts6) &&
            ...
            if (ptr->size() == 0) {
              ...
              for (int k = 0; k < ptr->size(); k++) {
                ...
                if (exsitingTrack->operator==(ptr_temp)) is_equal = true;
              }
...
std::vector<std::vector<HoughGridArea *> *> *GridInit2() {
  ...
  for (int i = 0; i < NAlpha; i++) {
    ...
    for (int j = 0; j < ND; j++) {
      auto ptr3 =
          new HoughGridArea(AlphaMin + i * AlphaBinWidth, DMin + j * DBinWidth,
                            DMin + (j + 1) * DBinWidth);
      ptr2->push_back(ptr3);
    }
    ptr1->push_back(ptr2);
...
void FillGrid2(std::vector<std::vector<HoughGridArea *> *> *gridMatrix,
               vector<HoughPoint *> &pointsList) {
  for (int ip = 0; ip < pointsList.size(); ip++) {
    auto point = pointsList.at(ip);
    for (int ia = 0; ia < gridMatrix->size(); ia++) {
      ...
      for (int id = 0; id < row->size(); id++) {
        ...
        double d =
            point->xConformal() * cos(alpha) + point->yConformal() * sin(alpha);
...
int main(int argc, char **argv) {
  ...
  string path =
      "/home/txiao/STCF_Oscar2.0.0/share/pi+/test2/root_data_source/pi+/posPt";
  path += argv[1];
  path += ".root";
  ...
  string savepath =
      "./trackdata_Pt" + string(argv[1]) + "_noise" + string(argv[2]) + ".root";
  ...
  for (int eventIDTest = eventIDTest_begin; eventIDTest < eventIDTest_end;
       eventIDTest++) {
    ...
    for (int ie = 0; ie < nevents; ie++) {
      ...
      if ((eventID == eventIDTest) && (trackID == 1) &&
          (eventID != eventID_skip)) {
        HoughPoint *ptr =
            new HoughPoint(posX, posY, posZ, eventID, trackID, layerID, Pt);
