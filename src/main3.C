...
int main(int argc, char **argv) {
  ...
  if (n_tracks_in_event != 0) savepath += "_multi" + string(argv[5]);
  savepath += ".root";
    ...
    for (auto ie : *test_set) {
      ...
      for (int ip = 0; ip < nhits; ip++) {
        if ((trackID->at(ip) == 1) && (eventID != eventID_skip)) {
          HoughPoint *ptr =
              new HoughPoint(posX->at(ip), posY->at(ip), posZ->at(ip), eventID,
  ...
  std::cout << "totoal tarcks useful: " << counts_useful_events << endl << endl;
  return 0;
}
