...
{
    auto h = new TH2D("h", "fill", NAlpha, AlphaMin, AlphaMax, ND, DMin, DMax);
    auto c1 = new TCanvas("c1", "Fill");
    for (int ip = 0; ip < pointsList.size(); ip++)
    {
        auto point = pointsList.at(ip);
        for (int ia = 0; ia < gridMatrix->size(); ia++)
    ...
    h->GetYaxis()->SetTitle("d [MeV^-1 c]");
    h->GetYaxis()->SetTitleOffset(2);
    h->GetZaxis()->SetTitle("Counts");
    c1->SaveAs("testgraph.png");
    c1->Close();
    delete h;
    delete c1;
}
...
int main(int argc, char **argv)
{
    ...
    for (int ie = 0; ie < nevents; ie++)
    {
        ...
        if (((eventID == eventIDTest) || (eventID == eventIDTest + 10) || (eventID == eventIDTest + 20)) && (trackID == 1) && (eventID != eventID_skip))
        {
    ...
    {
        double pt, Q;
        bool fine = tracks->at(i)->FitLinear(&pt, &Q);
        if (fine)
